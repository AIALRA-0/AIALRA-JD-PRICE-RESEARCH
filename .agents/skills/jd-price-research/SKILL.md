---
name: "jd-price-research"
description: "用于用户要求搜索 京东 当前商品、寻找最低价可行商品、核验在售价、拍卖价、成色、卖家、运费、退货、宣传图、评价和购买风险时，通过官方网页或已经连接且经过信任的 京东 读取后端完成多轮采集与详情核验；不用于下单、出价、议价、联系卖家、关注、导出登录信息、绕过验证或比较 京东 以外的平台"
---
...
只读取京东官方公开页面，不使用搜索摘要代替页面证据

只执行搜索、读取、筛选、核验和分析

禁止执行下单 加购 领券 关注 联系商家 评价或其他账户写入
...
`identity_phrases` 保存一至五个必须连续出现的型号写法，标题至少命中一个写法，同时还要包含 `required_terms` 的全部身份词
...
PLUS 会员价、地区价、以旧换新价和优惠券价必须分别标注条件
