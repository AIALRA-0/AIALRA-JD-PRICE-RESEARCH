...
<p>
  <a href="CHANGELOG.md"><img src="docs/images/badges/version.svg" alt="京东价格研究 Skill 当前版本 0.2.1"></a>
  <a href="#access-boundary"><img src="docs/images/badges/mode.svg" alt="京东价格研究 Skill 运行模式为只读"></a>
  <a href="#evidence-workflow"><img src="docs/images/badges/evidence.svg" alt="京东价格研究 Skill 使用多轮证据核验"></a>
...
这个仓库保存一个只读的 `jd-price-research` Codex Skill

用户要求研究 京东 时，Agent 会核验当前商品价格、详情页、图片、卖家、评价、价格条件和购买风险，然后输出可追溯结论
...
| 位置 | 用途 |
|---|---|
| `.agents/skills/jd-price-research/SKILL.md` | 定义触发条件和 Agent 必须遵守的运行规则 |
| `.agents/skills/jd-price-research/workflow.yaml` | 固定节点顺序、执行器、权限、失败路径和停止条件 |
| `.agents/skills/jd-price-research/schemas/` | 定义每个节点允许接收和返回的数据结构 |
| `.agents/skills/jd-price-research/scripts/` | 运行工作流、去重、排名或验证结果 |
| `.agents/skills/jd-price-research/references/` | 保存浏览器、多轮采集、风险和验收说明 |
| `tests/` | 验证成功路径和安全失败路径 |
...
```text
# 将下一行作为新的 Codex 任务
使用 $jd-price-research 多轮搜索并核验京东当前最低价可行商品
```
