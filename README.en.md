...
<p>
  <a href="CHANGELOG.md"><img src="docs/images/badges/version.svg" alt="JD Price Research Skill current version 0.2.1"></a>
  <a href="#2-read-only-and-security-boundaries"><img src="docs/images/badges/mode.svg" alt="JD Price Research Skill operates in read-only mode"></a>
  <a href="#3-how-it-works"><img src="docs/images/badges/evidence.svg" alt="JD Price Research Skill uses multi-round evidence verification"></a>
...
This repository contains the read-only `jd-price-research` Codex Skill

When a user asks for research on 京东, the Agent verifies current prices, detail pages, images, sellers, reviews, price conditions, and purchase risks and produces a traceable conclusion
...
Figure 3.1 Read-only evidence flow for JD Price Research Skill
...
| Location | Purpose |
|---|---|
| `.agents/skills/jd-price-research/SKILL.md` | Defines triggers and mandatory Agent behavior |
| `.agents/skills/jd-price-research/workflow.yaml` | Fixes node order, executors, permissions, failure paths, and stop conditions |
| `.agents/skills/jd-price-research/schemas/` | Defines the structures accepted and returned by every node |
| `.agents/skills/jd-price-research/scripts/` | Runs the workflow, deduplicates, ranks, or validates results |
| `.agents/skills/jd-price-research/references/` | Documents browser use, multi-round collection, risk, and acceptance rules |
...
Use $jd-price-research to search JD in multiple rounds and verify the current lowest feasible offer
```
